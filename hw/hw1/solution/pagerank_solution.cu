 ...
 * supporting kernel into a file called mp1-part3-solution.cu and submit
...
__global__
void device_graph_propagate(unsigned int *graph_indices, unsigned int *graph_edges, float *graph_nodes_in, float *graph_nodes_out, float *inv_edges_per_node,
                            int array_length)
{
    ...
    if (tid < array_length) {
        float sum = 0.f;
        for (int j = graph_indices[tid]; j < graph_indices[tid + 1]; ++j) {
            sum += graph_nodes_in[graph_edges[j]] * inv_edges_per_node[graph_edges[j]];
        }
        graph_nodes_out[tid] = 0.5f/(float)array_length + 0.5f * sum;
    }
}
                          ...
                          int avg_edges)
{
  ...
  cudaMalloc(&d_graph_indices,     (num_elements + 1)        * sizeof(unsigned int));
  cudaMalloc(&d_graph_edges,        num_elements * avg_edges * sizeof(unsigned int));
  cudaMalloc(&d_graph_nodes_A,      num_elements             * sizeof(float));
  cudaMalloc(&d_graph_nodes_B,      num_elements             * sizeof(float));
  cudaMalloc(&d_inv_edges_per_node, num_elements             * sizeof(float));
  ...
  cudaMemcpy(d_graph_nodes_A, h_graph_nodes_input, num_elements * sizeof(float), cudaMemcpyHostToDevice);
  ...
  for (int i = 0; i < nr_iterations; i += 2) {
      printf("iteration: %d\n", i);
      device_graph_propagate<<<(num_elements + block_size - 1) / block_size, block_size>>>(
              d_graph_indices, d_graph_edges, d_graph_nodes_A, d_graph_nodes_B, d_inv_edges_per_node, num_elements);
      device_graph_propagate<<<(num_elements + block_size - 1) / block_size, block_size>>>(
              d_graph_indices, d_graph_edges, d_graph_nodes_B, d_graph_nodes_A, d_inv_edges_per_node, num_elements);
  ...
  cudaMemcpy(h_graph_nodes_result, d_graph_nodes_A, num_elements * sizeof(float), cudaMemcpyDeviceToHost);
